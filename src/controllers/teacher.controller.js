...
export const addTeacher = async (req, res) => {
  ...
  try {
    ...
    const uploadOrNull = async (fileKey) =>
      files?.[fileKey]?.[0] ? await uploadToCloudinary(files[fileKey][0].path) : null;
    ...
    for (const fileKey in req.files) {
      const match = fileKey.match(/^courses\[(\d+)]\[certificate]$/);
      if (match) {
        const index = Number(match[1]);
        const uploaded = await uploadToCloudinary(req.files[fileKey][0].path);
        if (!courses[index]) courses[index] = {};
    ...
    for (const course of courses) {
      if (course.course_id && course.completion_date && course.expiration_date) {
        await connection.query(
          `INSERT INTO staff_courses (staff_id, course_id, completion_date, expiration_date, certificate_url)
           VALUES (?, ?, ?, ?, ?)`,
          [user_id, course.course_id, course.completion_date, course.expiration_date, course.certificate || null]
        );
      }
    }
